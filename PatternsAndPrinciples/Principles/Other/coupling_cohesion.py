...
class ExampleStart():
    ...
    def execute_logic(self, id):
        sensor_response = urllib.request.urlopen(f'{self.URL}/{id}')  # <-- dependency
...
class ExampleMiddle():
    def execute_logic(self, id):
        store = SensorStore()   # <-- dependency
        sensor = store.get_sensor(id)

        message  = 'alert' if sensor['data'] > 25 else 'ok'  # Actual business logic of the class

        message_sender = EmailSender()   # <-- dependency
